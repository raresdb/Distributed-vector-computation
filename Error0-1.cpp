...
void error0_1(int argc, char** argv) {
	...
	if(rank < 4) {
		...
		switch(rank) {
			...
			case 1:
				...
				send(rank, &cluster_workers.at(rank)[0], count, MPI_INT, 2, 0, MPI_COMM_WORLD);
				// receive from 2
				for(auto i = 0; i < 4; i++) {
					if(i == rank)
						continue;
					...
					cluster_workers.at(i).resize(count);
					MPI_Recv(&cluster_workers.at(i)[0] , count , MPI_INT , 2 , 0 , MPI_COMM_WORLD , &status);
				}
				break;
			case 2:
				// receive from 1 and send to 3
				MPI_Recv(&count , 1 , MPI_INT , 1 , 0 , MPI_COMM_WORLD , &status);
				cluster_workers.at(1).resize(count);
				MPI_Recv(&cluster_workers.at(1)[0] , count , MPI_INT , 1 , 0 , MPI_COMM_WORLD , &status);				
				for(auto i = 1; i < 3; i++) {
					count = cluster_workers.at(i).size();
					send(rank, &count, 1, MPI_INT, 3, 0, MPI_COMM_WORLD);
					send(rank, &cluster_workers.at(i)[0], count, MPI_INT, 3, 0, MPI_COMM_WORLD);
				}
				// receive from 3 and send to 1
				MPI_Recv(&count , 1 , MPI_INT , 3 , 0 , MPI_COMM_WORLD , &status);
				...
				for(auto i = 0; i < 4; i++) {
					if(i == 1)
						continue;
					count = cluster_workers.at(i).size();
					send(rank, &count, 1, MPI_INT, 1, 0, MPI_COMM_WORLD);
					send(rank, &cluster_workers.at(i)[0], count, MPI_INT, 1, 0, MPI_COMM_WORLD);
				}
				break;
			case 3:
	...
	// send it forward
	if(rank < 4 && rank > 1) {
		send(rank, &count, 1, MPI_INT, rank - 1, 0, MPI_COMM_WORLD);
		send(rank, &v[0], count, MPI_INT, rank - 1, 0, MPI_COMM_WORLD);
	}
	...
	if(rank < 4) {
		for(auto worker : cluster_workers.at(rank)) {
				...
				for(auto i = worker - 4; i < v.size(); i+= num_proc - 4) {
	...
	if(rank < 4) {
		for(auto wk : cluster_workers.at(rank)) {
			for(auto i = 0; i < worker_subvectors.at(wk).size(); i++) {
				v[i * (num_proc - 4) + wk - 4] = worker_subvectors.at(wk)[i];
			}
		}
	}
